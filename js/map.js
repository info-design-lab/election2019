var state = "ODISHA";
var constituency = "SAMBALPUR";
var unknownColor = "#e4e4e4";
var yearList = [1999, 2004, 2009, 2014, 2019, 2024];
var year = 2024; // Current Year
var map_legend;
var partyColors;
...
function makeMap(error, data, partyColors, mapCarto, mapSatellite){
    ...
    function updateMap(){
        map_svg.selectAll("path")  
            ...
            .style('fill', function(d) {  
                if (data[year] && data[year][constName(d)]) {  // ✅ Checks if data exists for the selected year
                    if (uncheckedParties.indexOf(data[year][constName(d)].Party) > -1) {
                        return "white";  // Hide party if unchecked
                    }
                    return getPartyColor(data[year][constName(d)].Party);  // ✅ Fetch correct color
                }
                return unknownColor;  // Default color for missing data
            });
    }
    ...
    function createYearSlider(){
        ...
        .append('circle')
        .attr('cx', function(d, i){
            return offsetX + i * 100;  // Positioning of circles
        })
        .attr('cy', offsetX)
        .attr('r', r)
        .attr('fill', "transparent")
        ...
        .attr('stroke-width', '2px')
        .on('click', function(d, i){  
            year = d;  // ✅ Updates selected year (including 2024)
            
            selectionCircle
                .transition().duration(500)
                .attr('cx', function(){
                    return offsetX + i * 100;  // ✅ Moves highlight circle
                });
    
            updateMap();  // ✅ Updates the map to show the selected year’s data
            createPartyLegend();  // ✅ Updates legend for the selected year
        });
