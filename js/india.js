...
function createIndiaVis(s){
    ...
    queue()
        .defer(d3.json, 'data/pipeline/india/indiaayush2019.json')
        .defer(d3.json, 'data/pipeline/india/indiaayush2024.json')
        .defer(d3.json, 'data/frontColors.json')
        .defer(d3.json, 'data/fronts2019.json')
        .defer(d3.json, 'data/fronts2024.json')
        .defer(d3.json, 'map/india/cartogram.json')
        .defer(d3.json, 'map/india/india.json')
        .await(makeIndiaVis);
}

function makeIndiaVis(error, data2019, data2024, frontColors, front2019, front2024, mapCarto, mapSatellite){
  	if(error){
  		console.log(error);
  	}
    ...
    var map1 = india1.selectAll("path")
        ...
        .style('fill', function(d) {
            ...
            let frontName = data2019[ST_CODE]?.[PC_CODE]?.["Front"];

            if (frontName && frontColors[frontName]) {
                legend_data2019[frontName] += 1;
                return frontColors[frontName];
            }
            
            return unknownColor;  // Default color if missing data
        })
    ...
    var map2 = india2.selectAll("path")
        ...
        .style('fill', function(d) {
            ...
            if(data2024[ST_CODE]){
                if(data2024[ST_CODE][PC_CODE]){
                    if(frontColors[data2024[ST_CODE][PC_CODE]["Front"]]){
                        legend_data2024[data2024[ST_CODE][PC_CODE]["Front"]] += 1;
                        return frontColors[data2024[ST_CODE][PC_CODE]["Front"]]
                    }
                }
            }
        ...
        .on('mouseover', function(d){
            ...
            if(data2024[ST_CODE]){
                if(data2024[ST_CODE][PC_CODE]["Front"]){
                    createSimpleTooltip(ST_CODE, PC_CODE);
                    india_tooltip1.style("visibility", "visible");
                    india_tooltip2.style("visibility", "visible");
                }
    ...
    function createPartyLegend(){
        var fronts = Object.keys(front2019);
    ...
    function createSimpleTooltip(ST_CODE, PC_CODE){
        ...
        var tipData = [
            {svg: india_tooltip1_svg, data: data2019},
            {svg: india_tooltip2_svg, data: data2024},
        ]  
