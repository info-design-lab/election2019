...
function makeMargin(error, mapSatellite) {
      ...
      if (d.properties.PC_NAME) {
        ...
        if (marginData[state][constName]) {
          if (marginData[state][constName][latestYear]) {
            const val =
              margin_mode === "votes"
                ? marginData[state][constName][latestYear].Margin
                : Math.round(
                    (marginData[state][constName][latestYear].Margin /
                      marginData[state][constName][latestYear]["Total Votes"]) *
                      10000
                  ) / 100;
  ...
  var margin = margin_cards.selectAll("circle").data(dlist).enter();

  var highlight_line = margin_cards
    .append("path")
  ...
  for (var i in yearList) {
    ...
    margin_tooltip[yearList[i]]
      ...
      .style("fill", "#696969")
      .attr("font-size", "20px") //votes winning
      .attr("class", "margin");
    margin_tooltip[yearList[i]]
      .append("text")
      .attr("x", 0)
      ...
      .attr("font-size", "20px") //winning party
      .attr("class", "winner");
    margin_tooltip[yearList[i]]
      .append("text")
      .attr("x", 0)
  ...
  for (var i in yearList) {
    margin_circles[yearList[i]] = margin
      .append("circle")
      .attr("cx", function (d) {
        if (d["value"][yearList[i]]) {
          if (margin_mode === "votes") {
            ...
            return scales[yearList[i]](
              d["value"][yearList[i]].Margin /
                d["value"][yearList[i]]["Total Votes"]
            );
          }
        }
        return 0;
  ...
  function getMarginScales() {
    for (var i in yearList) {
      if (marginData[state][Object.keys(marginData[state])[0]][yearList[i]]) {
        var val =
          margin_mode === "votes"
            ? marginData[state][Object.keys(marginData[state])[0]][yearList[i]]
                .Margin
            : marginData[state][Object.keys(marginData[state])[0]][yearList[i]]
                .Margin /
              marginData[state][Object.keys(marginData[state])[0]][yearList[i]][
      ...
      for (var j in marginData[state]) {
        if (marginData[state][j][yearList[i]]) {
          var val =
            margin_mode === "votes"
              ? marginData[state][j][yearList[i]].Margin
              : marginData[state][j][yearList[i]].Margin /
                marginData[state][j][yearList[i]]["Total Votes"];
          if (domain[0] >= val) {
            domain[0] = val;
          }
          if (domain[1] <= val) {
      ...
      if (domain) {
        scales[yearList[i]] = d3
          .scaleLinear()
          .domain(domain)
          .range([0, map_width * 0.9]);
      } else {
        scales[yearList[i]] = d3
          .scaleLinear()
          .domain([0, 0])
        ...
        if (marginData[state][constituency][yearList[i]]) {
          if (marginData[state][constituency][yearList[i - 1]]) {
            if (margin_mode === "votes") {
              path_array.push([
                scales[yearList[i]](
                  marginData[state][constituency][yearList[i]].Margin
                ),
                352 - i * 70 + 70 / 2,
              ]);
            } else {
              path_array.push([
                scales[yearList[i]](
                  marginData[state][constituency][yearList[i]].Margin /
                    marginData[state][constituency][yearList[i]]["Total Votes"]
                ),
                352 - i * 70 + 70 / 2,
              ]);
            }
  ...
  function updateTooltipText(t) {
    ...
    for (var i = 0; i < yearList.length; i++) {
        ...
        if (marginData[state][constituency][yearList[i]]) {
          ...
          if (margin_mode === "votes") {
            ...
            margin_tooltip[yearList[i]]
              ...
              .style(
                "fill",
                partyColors[marginData[state][constituency][yearList[i]].Party]
              )
            ...
            margin_tooltip[yearList[i]]
              ...
              .style(
                "fill",
                partyColors[marginData[state][constituency][yearList[i]].Runner]
              )
      ...
      if (!present) {
        margin_tooltip[yearList[i]].select(".margin").text("");
        margin_tooltip[yearList[i]].select(".winner").text("");
        margin_tooltip[yearList[i]].select(".runner").text("");
      }
    }
  }

  function updatePath() {
    highlight_line.datum(create_path(constituency)).attr(
      "d",
      d3
        .line()
        .x(function (d) {
  ...
  function getMarginLegendList() {
    ...
    for (var j in marginData[state]) {
      if (marginData[state][j][latestYear]) {
        val =
          margin_mode === "votes"
            ? marginData[state][j][latestYear].Margin
            : Math.round(
                (marginData[state][j][latestYear].Margin /
                  marginData[state][j][latestYear]["Total Votes"]) *
                  10000
              ) / 100;
    ...
    marginValueList.sort(function (a, b) {
      return b - a;
    });
    marginLegendList = [];
...
d3.selectAll('.year-label')
  .style('margin-bottom', '20px')  // Medium spacing of 20px
